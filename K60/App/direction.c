...
void DirectionControl(void)
{
	static float g_nDirectionErrorTemp[7];
        ...
        float GyroX = 0;
		g_nDirectionError = point[1]-point_y_set; //(int16)get_picture_diff(89, 1);//方向偏差
...
// 	else if(g_fSpeedFilter>3100){g_dirControl_P =470 ;g_dirControl_D = 800;}
	...
	g_fDirectionControlOut = g_nDirectionError*g_dirControl_P + g_fDirectionError_dot*g_dirControl_D;//+GyroX*g_dirControl_gyro_D;//PD转向
	 ...
	 g_fDirectionControlOut = (g_fDirectionControlOut > 900? 900: g_fDirectionControlOut);//转向输出限幅
	 g_fDirectionControlOut = (g_fDirectionControlOut < -900? -900: g_fDirectionControlOut);	
	 g_fDirectionControlOut = g_fDirectionControlOut*1+ g_fDirectionControlOut_last*0;
// 	 if (zhangai_time >= 1 && stop_flag_cnt >= 1000)
// 	 {
// 		 g_fDirectionControlOut = 130 * dir;
// 	 }
