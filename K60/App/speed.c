...
void MotorInit(void)
{
	...
	/* 初始化左电机PWM通道 */
	ftm_pwm_init(FTM3, FTM_CH0, 330, 0);
	ftm_pwm_init(FTM3, FTM_CH1, 330, 0);
	/* 初始化右电机PWM通道 */
	ftm_pwm_init(FTM3, FTM_CH2, 330, 0);
	ftm_pwm_init(FTM3, FTM_CH3, 330, 0);
	//初始化编码器输入口
...
void PWMOut(void)
{
	...
	g_PWMOut = g_fDirectionControlOut;// -g_fSpeedControlOut;
//	if (Flag_Speed == OFF)g_PWMOut = g_BlanceControlOut;		//如果Flag_Speed == OFF不输出速度

// 	if (g_fDirectionControlOut >= 0)
// 	{
		g_nLeftPWM = (int32)(g_PWMOut);
		g_nRighPWM = (int32)(g_PWMOut);
// 	}
// 	if (g_fDirectionControlOut<0)
// 	{
...
// 	}
	if (g_nLeftPWM < 0)
	{
		...
		nLeftPWM = (nLeftPWM > 1000 ? 1000 : nLeftPWM);
	...
	if (g_nRighPWM < 0)
	{
		...
		nRighPWM = (nRighPWM > 1000 ?1000 : nRighPWM);
